...
class fcnEV_Class:
    def __init__(self, prev_state, action, alpha, distance, cost_function):
        ...
        ##-----------------------------Calculate Costs-----------------------------------------------##
        self.energy_consumption = self.rewards_energy_efficiency_func()
    ...
    def rewards_energy_efficiency_func(self):
        ...
        if self.cost_function == CostFunction.ENERGY_EFFICIENCY:
            energy_consumption = (-1)*(self.battery_power__W * Constants.number_of_cells * self.delta_time_segment__s) / max_energy__Ws + (self.torque_requested__Nm / max_torque__Nm)**2 #* self.weight_lambda
